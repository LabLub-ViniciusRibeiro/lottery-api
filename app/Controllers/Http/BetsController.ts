...
export default class BetsController {
  ...
  public async store({ auth, request, response }: HttpContextContract) {
      ...
      const prices = await Promise.all(
        bets.map(async (bet) => {
          ...
          if (bet.chosenNumbers.length !== game.minMaxNumber) {
            return {
              message: `For the game ${game.type} you should choose ${game.minMaxNumber} numbers`,
            };
          }
          ...
          if (numberOutOfRange.length !== 0)
            return {
              message: `For the game ${game.type} you should choose numbers between 0 and ${game.range}`,
            };
          return game.price;
        })
      );
      const pricesFiltered = prices.filter(
        (price) => typeof price === "object"
      );

      if (pricesFiltered.length !== 0)
        return response.badRequest(pricesFiltered);

      const totalCart = prices.reduce(
        (previous, current) => (previous as number) + (current as number)
      );
      if (totalCart < (minCartValue as number)) {
        return response.badRequest({
          message: "Min cart value not reached",
          minValue: minCartValue,
          currentValue: totalCart,
        });
      }
    ...
    try {
      ...
      response.badRequest({
        message: "Error sending new bets mail",
