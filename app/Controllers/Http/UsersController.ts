...
export default class UsersController {
  ...
  //   try {
  //     if (noPaginate) {
  //       const users = await User.query().preload("roles", (roles) =>
  //         roles.select("name")
  //       );
  ...
  public async index({ auth, params, response }: HttpContextContract) {
    ...
    const userRoles = (await user.related("roles").query()).map(
      (role) => role.name
    );

    if (!userRoles.includes("admin")) {
      try {
        if (user.secureId !== userAuthenticated?.secureId)
    ...
    try {
      const userReturned = await User.query()
        .where("secure_id", user.secureId)
        // .preload('bets', (bets) => {
      ...
      response.ok(userReturned);
    } catch (error) {
      response.notFound({ message: error.message });
    }
  }

  public async changeUser({ auth, request, response }: HttpContextContract) {
    ...
    try {
      updatedUser = await User.findByOrFail("secure_id", user?.secureId);
